...
export class StoreManager {
  ...
  private getOrCreateGeminiService(
  ...
  async showStoreStats(): Promise<void> {
    ...
    try {
      ...
      modal.contentEl.createEl("p", {
        text: `Name: ${store.displayName}`,
      });
      modal.contentEl.createEl("p", { text: `ID: ${store.name}` });
      modal.contentEl.createEl("p", {
        text: `Created: ${new Date(store.createTime).toLocaleString()}`,
      });
      modal.contentEl.createEl("p", {
        text: `Updated: ${new Date(store.updateTime).toLocaleString()}`,
      });
      modal.open();
    } catch (err) {
