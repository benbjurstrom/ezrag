...
export class IndexStateStorageManager {
	...
	private readFromStorage(): IndexState {
		...
		try {
			...
			if (!this.isValidIndexState(parsed)) {
				console.warn('[IndexStateStorageManager] Invalid data structure in localStorage, using defaults');
				return this.deepClone(DEFAULT_INDEX_STATE);
			}

			return parsed;
		} catch (err) {
			console.error('[IndexStateStorageManager] Failed to read index state from localStorage', err);
			return this.deepClone(DEFAULT_INDEX_STATE);
		}
	}
	...
	private writeToStorage(state: IndexState): void {
		...
		try {
			...
			console.error('[IndexStateStorageManager] Failed to write index state to localStorage', err);
			// Check if quota exceeded
			if (err instanceof DOMException && err.name === 'QuotaExceededError') {
				console.error('[IndexStateStorageManager] localStorage quota exceeded! Index data too large.');
			}
		}
	}
	...
	clearState(): void {
		...
		try {
			...
			console.error('[IndexStateStorageManager] Failed to clear index state', err);
		}
	}
