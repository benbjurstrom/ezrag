...
import { DEFAULT_SETTINGS, IndexedDocState, IndexQueueEntry } from '../../src/types';
...
function createModal(options: ModalOptions = {}) {
  const stateManager = new StateManager({
    settings: { ...DEFAULT_SETTINGS, apiKey: options.apiKey ?? 'key', storeName: 'stores/current', storeDisplayName: 'Current' },
    index: {
      docs: options.docs ?? {},
      queue: options.queue ?? [],
    },
...
describe('StoreManagementModal rendering', () => {
  ...
  it('renders table rows with action buttons', async () => {
    ...
    const stores = [createStore('stores/current', 'Current'), createStore('stores/alt', 'Alt')];
    const setSpy = vi.spyOn(modal as any, 'handleSetCurrentStore').mockResolvedValue(undefined);
    const deleteSpy = vi.spyOn(modal as any, 'handleDeleteStore').mockResolvedValue(undefined);
...
describe('StoreManagementModal actions', () => {
  it('clears state and rebuilds when setting a new current store', async () => {
    ...
    const queue: IndexQueueEntry[] = [{ id: '1', vaultPath: 'Note.md', operation: 'upload', enqueuedAt: Date.now(), attempts: 0 }];
  ...
  it('deletes a non-current store after confirmation', async () => {
    ...
    const { modal, plugin } = createModal({ pluginOverrides: { storeManager } });
    vi.spyOn(modal as any, 'renderStoreTable').mockResolvedValue(undefined);
