...
describe("DocumentReplacer integration", () => {
  ...
  it("deletes an existing document before uploading a new copy", async () => {
    const gemini = {
      ...
      uploadDocument: vi
        .fn()
        .mockResolvedValue("fileSearchStores/demo/documents/123"),
    } as any;

    const replacer = new DocumentReplacer(gemini);

    const documentName = await replacer.replaceDocument(
      "fileSearchStores/demo/documents/old",
      {
    ...
    expect(gemini.deleteDocument).toHaveBeenCalledWith(
      "fileSearchStores/demo/documents/old",
    );
